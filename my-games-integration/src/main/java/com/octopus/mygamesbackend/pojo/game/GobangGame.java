package com.octopus.mygamesbackend.pojo.game;
...
public class GobangGame extends Game {
    ...
    public void handle(int x, int y, String username) {
        ...
        if (winner != -1) {
            ...
            WebSocketSessionPoolUtils.sendMessage(username, WSR.ok(1).put("data", playerDataMap.get(username)));
            WebSocketSessionPoolUtils.sendMessage(opponentName, WSR.ok(1).put("data", playerDataMap.get(opponentName)));
            try {
                Thread.sleep(5000);
